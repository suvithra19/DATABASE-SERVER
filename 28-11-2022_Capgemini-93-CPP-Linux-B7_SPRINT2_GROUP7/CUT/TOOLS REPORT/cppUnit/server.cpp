...
int AddToDataBase(SharedMemory &,empdata);
int DeleteFromDatabase(SharedMemory &,empdata);
int FindData(SharedMemory &,empdata,int &position);
int EditDataBase(SharedMemory &,empdata,int position);
...
void SocketServer::Accept ()
{
    ...
    while (1)
    {
        ...
        if (pid == 0)
        {
            ...
            recv(this->data_socket_fd,(empdata *)&(this->eData),sizeof(empdata), 0);
                ...
                case ADD:
                    {
                        ...
                        if(AddToDataBase(DataBase,this->eData) == FAILED_)
                ...
                case EDIT:
                    {
                        ...
                        if(FindData(DataBase,this->eData,position) == FAILED_)
                        {
                            send(this->data_socket_fd,(void*)FAILURE_MSG,MSG_SIZE,0);
                        }
                        else
                        {
                            ...
                            if(EditDataBase(DataBase,this->eData,position) == FAILED_)
                            {
                ...
                case DELETE:
                    {
                        ...
                        if(DeleteFromDatabase(DataBase,this->eData) == FAILED_)
                            send(this->data_socket_fd,(void*)FAILURE_MSG,MSG_SIZE,0);
                        else
                            send(this->data_socket_fd,(void*)SUCCESS_MSG,MSG_SIZE,0);
                        break;
...
int AddToDataBase(SharedMemory &DataBase,empdata eData)
{
    ...
    if (DataBase.AddData (eData) == SUCCESS)
    {
...
int FindData(SharedMemory &DataBase,empdata eData,int &position)
{
    ...
    if (DataBase.FindData(eData,position) == SUCCESS)
        return SUCCESS;
    if (DataBase.DetachFromMemory() == FAILED_)
        cout <<"[INFO]: Detaching from the shared memory Failed" <<endl;
    return FAILED_;
...
int EditDataBase(SharedMemory &DataBase,empdata eData,int position)
{
    ...
    if (DataBase.EditData(eData,position) == SUCCESS)
    {
...
int DeleteFromDatabase(SharedMemory &DataBase,empdata eData)
{
    ...
    if (DataBase.DeleteData (eData) == SUCCESS)
    {
